...
# Log
LOG_PATH = join(dirname(dirname(__file__)), 'data', 'multiple_image_results.csv')
...
IMAGE_PATH_ACC = join(dirname(dirname(__file__)), 'images', 'multiple_image_results_acc.png')
IMAGE_PATH_SIM = join(dirname(dirname(__file__)), 'images', 'multiple_image_results_sim.png')
IMAGE_PATH_ITER = join(dirname(dirname(__file__)), 'images', 'multiple_image_results_iter.png')
results = pd.read_csv(LOG_PATH)

sns.relplot(data=results, x='noise', y='success', col='label', col_wrap=3, kind='line', hue='n_patterns', palette='deep', markers=True, dashes=False, ci=None, style='label')
plt.savefig(IMAGE_PATH_ACC)
plt.clf()
sns.relplot(data=results, x='noise', y='similarity', col='label', col_wrap=3, kind='line', hue='n_patterns', palette='deep', markers=True, dashes=False, ci=None, style='label')
plt.savefig(IMAGE_PATH_SIM)
plt.clf()
sns.relplot(data=results, x='noise', y='iterations', col='label', col_wrap=3, kind='line', hue='n_patterns', palette='deep', markers=True, dashes=False, ci=None, style='label')
